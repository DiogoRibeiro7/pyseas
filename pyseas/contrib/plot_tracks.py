...
def _add_subpanel(
    ...
    if not show_xticks:
        ...
        lbls = [f"{x.replace(tzinfo=None):%Y-%m-%d:%H:%M}" for x in stamps]
        ax.set_xticks(ticks)
...
PlotPanelInfo = namedtuple(
    "PlotFishingPanelInfo", ["map_ax", "plot_axes", "projection_info", "legend_handles"]
)
...
def plot_panel(
    ...
    map_ratio=5.0,
    annotations=0,
    annotation_y_loc=1.0,
    annotation_y_align="bottom",
    annotation_axes_ndx=0,
    add_night_shades=False,
    projection_info=None,
    shift_by_cent_lon=None,
    label_angle=30,
    gs=None,
):
    ...
    ax1 = maps.create_map(
        gs[0], projection=projection_info.projection, extent=projection_info.extent
    )
    ax1.set_anchor("S")
    ...
    handles = maps.add_plot(lon, lat, kind, ax=ax1, props=prop_map, break_on_change=break_on_change)

    axes = []
    for i, plot_descr in enumerate(plots):
        offset = (
            None
            if (plot_descr["label"] not in shift_by_cent_lon)
            else (180 - projection_info.central_longitude)
        )
...
def track_state_panel(
    ...
    return plot_panel(
        ...
        annotation_y_loc=annotation_y_loc,
        annotation_y_align=annotation_y_align,
        annotation_axes_ndx=annotation_axes_ndx,
        add_night_shades=add_night_shades,
        projection_info=projection_info,
        shift_by_cent_lon=shift_by_cent_lon,
        label_angle=label_angle,
        gs=gs,
    )
...
def multi_track_panel(
    ...
    if prop_map is None and isinstance(prop_map, str):
        prop_map = plt.rcParams.get(prop_map)
...
def plot_tracks_panel(
    ...
    if plots is None:
        plots = [{"label": "longitude", "values": lon}, {"label": "latitude", "values": lat}]
    return plot_panel(
        timestamp,
        lon,
        lat,
