...
custom_artist_cycler = cycler(edgecolor=colors, facecolor=[(0, 0, 0, 0)] * len(colors))
...
def get_hourly_positions(gap_id, gaps_data, hourly_data):
    ...
    df = hourly_data[
        (hourly_data.ssvid == gap.ssvid)
        & (hourly_data.gap_id == gap_id)
        & (hourly_data.date.dt.date >= query_start.date())
        & (hourly_data.date.dt.date <= query_end.date())
    ].copy()
    df.drop(
        df[(df.date.dt.date == query_start.date()) & (df.hour < query_start.hour)].index,
        inplace=True,
    )
    df.drop(
        df[(df.date.dt.date == query_end.date()) & (df.hour > query_end.hour)].index, inplace=True
    )
...
def plot_table_basic(all_gaps):
    ...
    gap_implied_speed = []
    gap_reception = []
    for _index, gap in all_gaps.iterrows():
        gap_hours.append(f"{gap.gap_hours:0.2f}")
        gap_offon_class.append(f"{gap.off_class}/{gap.on_class}")
        gap_implied_speed.append(f"{gap.gap_implied_speed_knots:0.2f}")
        gap_reception.append(f"{gap.positions_per_day_off:0.2f}/{gap.positions_per_day_on:0.2f}")
    ...
    bbox = (
        [0.3, 0.3, 0.3, 1]
        if num_gaps == 1
        else ([0.3, 0.3, 0.5, 1.0] if num_gaps < 4 else [0.2, 0.3, 0.8, 1.0])
    )
    table = plt.table(cellText=cell_text, rowLabels=rows, loc="center", bbox=bbox)
...
def plot_table_ee(all_gaps):
    ...
    for _index, gap in all_gaps.iterrows():
        ...
        gap_implied_speed.append(f"{gap.gap_implied_speed_knots:0.2f}")
        gap_reception.append(f"{gap.positions_per_day_off:0.2f}/{gap.positions_per_day_on:0.2f}")
        gap_ee_pos.append(f"{gap.ee_positions}")
        gap_ee_pos_invalid.append(f"{gap.ee_positions_invalid}")
        gap_ee_pos_4hr.append(f"{gap.ee_positions_over_4hr}")
        gap_max_ee_gap.append(f"{gap.max_ee_gap:0.2f}")
        gap_max_gap.append(f"{gap.max_gap:0.2f}")
    ...
    rows = [
        ...
        "PPD at off/on",
        "EE Positions",
        "EE Positions - 4hr buffer",
        "Max EE Gap Length (h)",
        "Max Gap Length (h)",
        "No EE in gap (4hr buffer) & max gap>=12h",
        "Max gap>=24h & max gap>0.5*gap hours",
    ]
    ...
    bbox = (
        [0.35, -0.6, 0.3, 2.0]
        if num_gaps == 1
        else ([0.3, -0.6, 0.5, 3.0] if num_gaps < 4 else [0.2, -0.6, 0.8, 3.0])
    )
    table = plt.table(cellText=cell_text, rowLabels=rows, loc="center", bbox=bbox)
    table.set_fontsize(12)
...
def plot_gap(
    ...
    if table_type not in ["none", "basic", "ee"]:
        raise ValueError(
            f"{table_type!r} is not a valid table_type. Choose from 'none', 'basic', or 'ee'."
        )
    ...
    if df_positions_gfw.empty:
        raise RuntimeError(
            f"An image cannot be produced for gap {gap_id} because there are no Spire/Orbcomm positions for this dataset. This is usually due to vessels being left out of the `pipe_vYYYYMMDD_fishing` database."
        )
    ...
    if show_all_gaps:
        df_all_gaps = (
            gaps_data[
                (gaps_data.ssvid == gap_info.ssvid)
                & (gaps_data.gap_start >= df_positions_gfw.timestamp.min())
                & (gaps_data.gap_end <= df_positions_gfw.timestamp.max())
            ]
            .sort_values("gap_start")
    ...
    with (
        pyseas.context(pyseas.styles.light),
        pyseas.context(
            {"pyseas.eez.bordercolor": "black", "pyseas.map.trackprops": custom_artist_cycler}
        ),
    ):
        ...
        extent = [
            df_positions_all.lon.min(),
            df_positions_all.lon.max(),
            df_positions_all.lat.min(),
            df_positions_all.lat.max(),
        ]
        ...
        maps.create_map(
            ...
            extent=[
                extent[0] - im_buffer_lon,
                extent[1] + im_buffer_lon,
                extent[2] - im_buffer_lat,
                extent[3] + im_buffer_lat,
            ],
        )
        ...
        plt.gca().scatter(
            df_positions_all.iloc[0].lon,
            df_positions_all.iloc[0].lat,
            transform=maps.identity,
            color=start_color_face,
            edgecolor=start_color_edge,
            ls="-",
            lw=1,
            s=180,
            marker="*",
        ...
        sat_lon, sat_lat = (
            df_positions_all[df_positions_all.receiver_type == "satellite"].lon,
            df_positions_all[df_positions_all.receiver_type == "satellite"].lat,
        )
        terr_lon, terr_lat = (
            df_positions_all[df_positions_all.receiver_type == "terrestrial"].lon,
            df_positions_all[df_positions_all.receiver_type == "terrestrial"].lat,
        )
        ...
        ### Legend and title
        plt.gca().legend(
            loc="center left", bbox_to_anchor=(1, 0.5), facecolor="#f7f7f7", framealpha=1.0
        )
        ...
        df_gap_hourly = df_gap_hourly.assign(
            timestamp=pd.to_datetime(df_gap_hourly.date, format="%Y%m%d %H%M%S")
        ...
        plt.plot(
            df_gap_hourly.timestamp.values,
            df_gap_hourly.sat_positions.values,
            color=sat_color,
            label="Satellite",
            zorder=3,
        )
        plt.plot(
            df_gap_hourly.timestamp.values,
            df_gap_hourly.ter_positions.values,
            color=ter_color,
            label="Terrestrial",
            zorder=2,
        )
        if has_ee:
            plt.plot(
                df_gap_hourly.timestamp.values,
                df_gap_hourly.ee_positions.values,
                color=ee_color,
                label="Exact Earth",
                zorder=1,
            )
        ...
        plt.axvspan(
            ...
            ls="-",
            lw=0.7,
            edgecolor=gap_fig_color_edge,
            facecolor=gap_fig_color_face,
            alpha=0.5,
            zorder=0,
            label="Gap" if "Gap" not in plt.gca().get_legend_handles_labels()[1] else "",
        )
        ...
        for _index, gap in df_all_gaps.iterrows():
            plt.axvspan(
                gap.gap_start,
                gap.gap_end,
                ls="-",
        ...
        if table_type != "none":
            ...
            y_offsets = (
                ...
                if table_type == "none"
                else [1.2, 1.0, 0.8, 0.6, 0.4, 0.2]
            )
            plt.text(
                x_offset,
                y_offsets[0],
                f"Vessel: {gap_info.vessel_class}",
            ...
            if performance_data is not None:
                ...
                plt.text(
                    x_offset,
                    y_offsets[2],
                    f"Actual Class: {vessel_perf.actual_class}",
                    transform=plt.gca().transAxes,
                    fontsize="14",
                )
                plt.text(
                    x_offset,
                    y_offsets[3],
                    f"Avg Sat. PPD: {vessel_perf.avg_sat_positions_per_day:0.2f}",
                    transform=plt.gca().transAxes,
                    fontsize="14",
                )
                plt.text(
                    x_offset,
                    y_offsets[4],
                    f"Avg Expected PPD: {vessel_perf.avg_expected_positions_per_day:0.2f}",
                    transform=plt.gca().transAxes,
                    fontsize="14",
                )
                plt.text(
                    x_offset,
                    y_offsets[5],
                    f"Ratio actual/exp.: {vessel_perf.ratio_actual_to_expected:0.2f}",
                    transform=plt.gca().transAxes,
                    fontsize="14",
                )
